...
def main(username, out_folder="/soup_io_images/", since="0"): # as base of coding, used this: http://stackoverflow.com/questions/257409/download-image-file-from-the-html-page-source-using-python
    ...
    while(page<=maxPages):
        ...
        #write info
        log("[{0}] Getting images for page: {1}/{2} (max), from: {3}, for url: {4}. (Last date: {5})\n".format(datetime.datetime.now(),page, maxPages, timeStamp, fullUrl,lastTimeOfPost), True)
        ...
        try:
            ...
            continue
        #download images
        downloadImages(soup,parsed,out_folder, strstarttime)
...
def downloadImages(soup, parsed, out_folder, strstarttime):
...
if __name__ == "__main__":
    ...
    if username==None or username=="" or username==" ":
        ...
        exit()
    main(username, main_folder, since_date)